...
# After reboot: SET YOUR PASSWORD!
...
1. **Forgetting to set a password** - The flake uses empty passwords by default. You CANNOT log in without setting a password with `sudo passwd username` or adding SSH keys.

2. **Not mounting /boot** - If you skip `mkdir -p /mnt/boot && mount /dev/sda1 /mnt/boot`, your system won't boot.

3. **Wrong device name** - Always verify with `lsblk` before partitioning. NVMe drives use `/dev/nvme0n1` not `/dev/sda`.

4. **Using the example machine config for first install** - For your first install, use the pre-built profiles directly (e.g., `#gnome-workstation`). Create custom machine configs later.

5. **Forgetting to import hardware-configuration.nix** - When creating custom machine configs, you must import the hardware config generated by `nixos-generate-config`.

6. **Cloning to wrong location** - Clone to `/mnt/nixos-base` (not `/tmp` or your home directory) so the flake path works during installation.

7. **Not checking disk space** - Ensure at least 20GB free space. Desktop environments require substantial storage.

8. **Forgetting experimental features** - If you get "experimental feature 'nix-command' is disabled", use `nix --extra-experimental-features 'nix-command flakes' <command>`
...
- [ ] You can log in (password set or SSH keys work)
- [ ] Display manager loads (for desktop profiles)
- [ ] Desktop environment starts
- [ ] WiFi/network connectivity works
- [ ] Audio works: `speaker-test -t wav -c 2`
- [ ] Basic commands available: `git --version`, `nvim --version`
- [ ] Home Manager activated: `home-manager generations`
- [ ] Flakes enabled: `nix flake show /etc/nixos` (or wherever you cloned)
- [ ] Can rebuild: `sudo nixos-rebuild switch --flake /etc/nixos#<config>`

**If anything fails**, check [TROUBLESHOOTING.md](TROUBLESHOOTING.md) or the issues on GitHub.
